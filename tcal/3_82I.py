...
def funcion_objetivo(l):
  ...
  area_exterior = calculate_cylinder_area(g_d_exterior, l)
  ...
  resistencia_cobre = calculate_conduction_thermal_resistance_for_cylinder(
      g_d_interior / 2, g_d_exterior / 2, l, g_k_cobre)

  resistencia_exterior = calculate_convection_thermal_resistance(g_h_externa, area_exterior)

  resistencia_total = sum_linear_resistances([resistencia_interior, resistencia_cobre, resistencia_exterior])

  return (g_t_interior - g_t_exterior) / resistencia_total - g_hv_agua * g_entrada_agua


def main():
  print(f"se ocupan {regula_falsi(500, 2000, funcion_objetivo)} pies de cobre")
